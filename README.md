...
- **📝 字幕提取** — 优先提取内嵌字幕，自动 fallback 到 ASR 语音识别（Whisper API / Qwen DashScope）
...
└── frontend        — Vite + React build，由 Node 静态服务提供并代理 /api (:3000)
```
...
- ASR API Key（如果视频没有可用内嵌字幕，则需要 Whisper API 或 Qwen DashScope）
...
# 访问令牌（可选；当前用于 /api/auth/verify 和前端 token 校验，至少 32 字符）
# ACCESS_TOKEN=<随机生成>
...
| AI | Anthropic SDK 兼容接口, Whisper API / Qwen DashScope |
| 视频 | ffmpeg, ffprobe |
| 基础设施 | Docker, MySQL, Redis, Node.js 前端静态服务 |
...
| `ACCESS_TOKEN` | 可选访问令牌，当前用于 `/api/auth/verify` 和前端 token 校验 | |
...
| `APP_SETTINGS_PATH` | 服务端设置文件路径，默认 `data/settings.json` | |
| `APP_PRESETS_PATH` | API provider 预设文件路径，默认 `data/presets/api_providers.json` | |
| `MAX_VIDEO_DURATION_SECONDS` | 上传视频时长上限（秒），默认 43200 | |
| `RETENTION_DAYS` | Celery Beat 清理已过期任务的保留天数，默认 7 | |
| `CHAT_LOCK_TTL` | 单任务聊天锁 TTL（秒），默认 300 | |
| `CHAT_TOOL_RESULT_MAX_CHARS` | 聊天中工具结果注入上下文的最大字符数，默认 4000 | |
| `RATE_LIMIT_AUTH_FAIL` | AuthMiddleware 启用时的认证失败限流阈值，默认 10 | |
| `RATE_LIMIT_UPLOAD` | 上传限流阈值，默认 5 | |
| `RATE_LIMIT_CHAT` | 聊天限流阈值，默认 20 | |
| `RATE_LIMIT_OTHER` | 其他 API 限流阈值，默认 120 | |
| `LOG_FORMAT` | 日志格式: `json` 或 `text`；留空按运行环境选择 | |
| `PRESETS_PATH` | 字幕样式预设文件路径覆盖项 | |
...
- `ACCESS_TOKEN` 当前用于前端 token 校验端点；API 路由级强制鉴权尚未接入全局中间件
- **路径穿越防护** — 中间件拦截 `..` 和编码绕过
