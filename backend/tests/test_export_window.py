...
def test_preserves_highlight_when_no_budget():
    ...
    s, e = _compute_export_window(
        10, 130, buffer=3, max_duration=120, video_duration=7200
    )
...
def test_full_buffer_when_budget_available():
    ...
    s, e = _compute_export_window(
        10, 50, buffer=5, max_duration=120, video_duration=7200
    )
...
def test_boundary_clip_no_buffer():
    ...
    s, e = _compute_export_window(
        0, 120, buffer=5, max_duration=120, video_duration=7200
    )
...
def test_start_boundary_limits_before_buffer():
    ...
    s, e = _compute_export_window(
        0, 50, buffer=10, max_duration=120, video_duration=7200
    )
...
def test_end_boundary_limits_after_buffer():
    ...
    s, e = _compute_export_window(
        7100, 7150, buffer=10, max_duration=120, video_duration=7200
    )
    assert s == 7090
...
def test_buffer_symmetric_within_budget():
    ...
    s, e = _compute_export_window(
        100, 160, buffer=5, max_duration=120, video_duration=7200
    )
    assert s == 95
    assert e == 165
    assert e - s == 70
...
def test_large_buffer_exceeds_budget():
    ...
    s, e = _compute_export_window(
        100, 130, buffer=30, max_duration=60, video_duration=7200
    )
