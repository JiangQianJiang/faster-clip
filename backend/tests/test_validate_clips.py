...
def test_caps_to_three():
    ...
    result = validate_clips(clips, video_duration=300, min_duration=30, max_duration=120)
...
def test_resolves_overlaps():
    ...
    for i in range(len(result)):
        for j in range(i + 1, len(result)):
            ...
            overlap = a["start_time_s"] < b["end_time_s"] and b["start_time_s"] < a["end_time_s"]
...
def test_expand_short_clip_within_bounds():
    ...
    result = validate_clips(clips, video_duration=200, min_duration=30, max_duration=120)
    assert len(result) == 1
